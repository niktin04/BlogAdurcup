...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--templates', '-t', action='store', dest='app_template',
            help='The path to the app templates')
        parser.add_argument(
    ...
    def handle(self, *args, **options):
        ...
        warnings.warn(
            ...
            "be removed in future releases. Use \"startapp --templates\" instead.",
            MarkedForDeprecationWarning
        )
        if django.VERSION[:2] >= (1, 10):
            raise CommandError("This command is deprecated. Please use \"startapp --templates\" instead.")
        ...
        if not os.path.exists(app_template):
            raise CommandError("The templates path, %r, does not exist." % app_template)
...
def copy_template(app_template, copy_to, project_name, app_name):
    """copies the specified templates directory to the copy_to location"""
    import shutil

    app_template = os.path.normpath(app_template)
    # walks the templates structure and copies it
